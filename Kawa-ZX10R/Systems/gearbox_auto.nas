...
var speedlimstate = props.globals.getNode("/instrumentation/Kawa-ZX10R/speed-indicator/speed-limiter-switch");
var speed = 0;
var gspeed = 0;
var ascon = props.globals.initNode("/controls/Kawa-ZX10R/ASC/on-off",1,"BOOL");
...
var loop = func {
	...
	# second possibility for clutch control backspace key is setting in the Kawa-ZX10R1000-gears-set.xml
	...
	if(running.getValue() == 1){
		...
		if(gspeed < 40){
			...
			gear.setValue(4);
			vmax = 130;
		}else if(gspeed > 100 and gspeed < 140){
			gear.setValue(5);
			vmax = 170;
		}else if(gspeed > 140){
			gear.setValue(6);
		...
		speed = getprop("/instrumentation/Kawa-ZX10R/speed-indicator/speed-meter");
		...
		if(comp_m < 0.06 and brake_ctrl_right <= 0.5 and brake_ctrl_left <= 0.5 and gspeed > 70 and ascon.getValue() == 1){
			...
			setprop("/controls/Kawa-ZX10R/ASC/ctrl-light", 1);
		}else{
			setprop("/controls/Kawa-ZX10R/ASC/ctrl-light", 0);
		}
	...
	if(comp_m < 0.05 and brake_ctrl_right > 0.5 and brake_ctrl_left > 0.5 and gspeed > 70){
		setprop("/controls/Kawa-ZX10R/ABS/ctrl-light", 1);
		setprop("/controls/Kawa-ZX10R/ABS/brake-right", brake_ctrl_right*0.34);
		setprop("/controls/Kawa-ZX10R/ABS/brake-left", brake_ctrl_left*0.34);		
	}else{
		setprop("/controls/Kawa-ZX10R/ABS/ctrl-light", 0);
		setprop("/controls/Kawa-ZX10R/ABS/brake-right", brake_ctrl_right);
		setprop("/controls/Kawa-ZX10R/ABS/brake-left", brake_ctrl_left);
	}
