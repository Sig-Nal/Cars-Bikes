...
var forkcontrol = func{
	...
	if(getprop("/instrumentation/Honda-CBR/speed-indicator/selection")){
		setprop("/instrumentation/Honda-CBR/distance-calculator/miles", getprop("/instrumentation/Honda-CBR/distance-calculator/mzaehler")*0.621371192); # miles on bike
		setprop("/instrumentation/Honda-CBR/distance-calculator/dmiles", getprop("/instrumentation/Honda-CBR/distance-calculator/dmzaehler")*0.621371192); # miles a day
	}else{
		setprop("/instrumentation/Honda-CBR/distance-calculator/miles", getprop("/instrumentation/Honda-CBR/distance-calculator/mzaehler")); # km on bike
		setprop("/instrumentation/Honda-CBR/distance-calculator/dmiles", getprop("/instrumentation/Honda-CBR/distance-calculator/dmzaehler")); # km a day
	}
...
var temp_fake_calc = func{
	...
	interpolate("/engines/engine[0]/engine-temperatur", net, 20);
	settimer(temp_fake_calc, 20);
};
...
setlistener("/surface-positions/left-aileron-pos-norm", func{
	...
	var omm = getprop("/controls/Honda-CBR/old-mens-mode") or 0;
	...
	var driverpos = getprop("/controls/Honda-CBR/driver-up") or 0;
...
setlistener("/gear/gear/rollspeed-ms", func (speed){
	...
	if(getprop("/instrumentation/Honda-CBR/speed-indicator/selection")){
		if(speed > 0.1){
			...
			setprop("/instrumentation/Honda-CBR/speed-indicator/speed-meter", 0);
		}
	}else{
		if(speed > 0.1){
			setprop("/instrumentation/Honda-CBR/speed-indicator/speed-meter", speed*3600/1000); # km/h
		}else{
			setprop("/instrumentation/Honda-CBR/speed-indicator/speed-meter", 0);
		}
	}
...
setlistener("/instrumentation/Honda-CBR/speed-indicator/speed-limiter", func (sl){
...
setlistener("sim/model/start-idling", func()
 ...
 setlistener("/controls/gear/brake-parking", func(p)
  ...
  if (p.getValue() == 1)
   {
   		interpolate("/controls/Honda-CBR/driver-up", 1, 0.8);
   }
  else
   {
   		interpolate("/controls/Honda-CBR/driver-up", 0, 0.8);
   }
  }, 1, 0);
