...
var speedlimiter = props.globals.getNode("/instrumentation/Honda-CBR/speed-indicator/speed-limiter");
var speedlimstate = props.globals.getNode("/instrumentation/Honda-CBR/speed-indicator/speed-limiter-switch");
var speed = 0;
var gspeed = 0;
var ascon = props.globals.initNode("/controls/Honda-CBR/ASC/on-off",1,"BOOL");
...
var loop = func {
	...
	var gefahrenem = getprop("/instrumentation/Honda-CBR/distance-calculator/mzaehler") or 0;
	var tagesm = getprop("/instrumentation/Honda-CBR/distance-calculator/dmzaehler") or 0;
	gefahrenem = gefahrenem + msec/8*1.16;  # 0.125 sec * 8 / 1.16 correction value for the wheel dimension
	tagesm = tagesm + msec/8*1.16;
	setprop("/instrumentation/Honda-CBR/distance-calculator/mzaehler", gefahrenem);
	setprop("/instrumentation/Honda-CBR/distance-calculator/dmzaehler", tagesm);
	...
	# second possibility for clutch control backspace key is setting in the Honda-CBR1000RR-set.xml
	...
	if(running.getValue() == 1){
		...
		speed = getprop("/instrumentation/Honda-CBR/speed-indicator/speed-meter");
		...
		if(rpm.getValue() > maxrpm-750){
			if(engine_rpm_regulation.getValue() == 1 ){
				...
				setprop("/controls/Honda-CBR/ctrl-light-overspeed", 1);
			}else{
				setprop("/controls/Honda-CBR/ctrl-light-overspeed", 1);
			}
			
		}else{
			setprop("/controls/Honda-CBR/ctrl-light-overspeed", 0);
		}
		...
		if(comp_m < 0.06 and brake_ctrl_right <= 0.5 and brake_ctrl_left <= 0.5 and gspeed > 70 and ascon.getValue() == 1){
			...
			setprop("/controls/Honda-CBR/ASC/ctrl-light", 1);
		}else{
			setprop("/controls/Honda-CBR/ASC/ctrl-light", 0);
		}
	...
	if(comp_m < 0.05 and brake_ctrl_right > 0.5 and brake_ctrl_left > 0.5 and gspeed > 70){
		setprop("/controls/Honda-CBR/ABS/ctrl-light", 1);
		setprop("/controls/Honda-CBR/ABS/brake-right", brake_ctrl_right*0.34);
		setprop("/controls/Honda-CBR/ABS/brake-left", brake_ctrl_left*0.34);		
	}else{
		setprop("/controls/Honda-CBR/ABS/ctrl-light", 0);
		setprop("/controls/Honda-CBR/ABS/brake-right", brake_ctrl_right);
		setprop("/controls/Honda-CBR/ABS/brake-left", brake_ctrl_left);
	}
