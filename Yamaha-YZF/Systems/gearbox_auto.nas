...
var loop = func {
	...
	# second possibility for clutch control backspace key is setting in the Yamaha-YZF-gears-set.xml
	...
	if(running.getValue() == 1){
		...
		if(comp_m_f < 0.06 or comp_m_r < 0.06 and brake_ctrl_right <= 0.5 and brake_ctrl_left <= 0.5){
			...
			setprop("/controls/Yamaha-YZF/ASR/ctrl-light", 1);
		}else{
			setprop("/controls/Yamaha-YZF/ASR/ctrl-light", 0);
		}
