...
var forkcontrol = func{
	...
	if(getprop("/instrumentation/Honda-NSF/speed-indicator/selection")){
		setprop("/instrumentation/Honda-NSF/distance-calculator/miles", getprop("/instrumentation/Honda-NSF/distance-calculator/mzaehler")*0.621371192); # miles on bike
		setprop("/instrumentation/Honda-NSF/distance-calculator/dmiles", getprop("/instrumentation/Honda-NSF/distance-calculator/dmzaehler")*0.621371192); # miles a day
	}else{
		setprop("/instrumentation/Honda-NSF/distance-calculator/miles", getprop("/instrumentation/Honda-NSF/distance-calculator/mzaehler")); # km on bike
		setprop("/instrumentation/Honda-NSF/distance-calculator/dmiles", getprop("/instrumentation/Honda-NSF/distance-calculator/dmzaehler")); # km a day
	}
...
setlistener("/surface-positions/left-aileron-pos-norm", func{
	...
	var omm = getprop("/controls/Honda-NSF/old-mens-mode") or 0;
	...
	var driverpos = getprop("/controls/Honda-NSF/driver-up") or 0;
...
setlistener("/gear/gear/rollspeed-ms", func (speed){
	...
	if (speed > 3) setprop("/controls/waiting", 1);
	if(getprop("/instrumentation/Honda-NSF/speed-indicator/selection")){
		if(speed > 0.1){
			setprop("/instrumentation/Honda-NSF/speed-indicator/speed-meter", speed*3600/1000*0.621371); # mph
		}else{
			setprop("/instrumentation/Honda-NSF/speed-indicator/speed-meter", 0);
		}
	}else{
		if(speed > 0.1){
			setprop("/instrumentation/Honda-NSF/speed-indicator/speed-meter", speed*3600/1000); # km/h
		}else{
			setprop("/instrumentation/Honda-NSF/speed-indicator/speed-meter", 0);
		}
	}
...
setlistener("sim/model/start-idling", func()
 ...
 setlistener("/controls/gear/brake-parking", func(p)
  ...
  if (p.getValue() == 1)
   {
   		interpolate("/controls/Honda-NSF/driver-up", 1, 0.8);
   }
  else
   {
   		interpolate("/controls/Honda-NSF/driver-up", 0, 0.8);
   }
  }, 1, 0);
