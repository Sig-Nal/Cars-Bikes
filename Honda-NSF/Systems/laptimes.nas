...
var racelap = props.globals.getNode("/Honda-NSF/race-lap");
var thissector = props.globals.getNode("/Honda-NSF/this-sector");
# only for mp result view
var sectortime = props.globals.getNode("/Honda-NSF/this-sector-time");
var laptime = props.globals.getNode("/Honda-NSF/this-lap-time");
var racetime = props.globals.getNode("/Honda-NSF/this-race-time");
var inrange = 0;
...
var total_result = func{
	var maxlaps = getprop("/Honda-NSF/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/Honda-NSF/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/Honda-NSF/show-race-times") or 0;
	var show_l_or_f = getprop("/Honda-NSF/switch-last-fastest") or 0;
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/Honda-NSF/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/Honda-NSF/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/Honda-NSF/"~pa~"/sector["~n~"]/last-time") or 0;
		var ft  = getprop("/Honda-NSF/"~pa~"/sector["~n~"]/fastest-time") or 0;
		...
		if(thsec == n and racelap.getValue() > 0){
			...
			if(ct > 0){
				...
				setprop("Honda-NSF/this-sector-time-sec", result[0]);
				setprop("Honda-NSF/this-sector-time-min", result[1]);
			}
			if(ft > 0){
				var result = calc_time(ft);
				setprop("Honda-NSF/fastest-sector-time-sec", result[0]);
				setprop("Honda-NSF/fastest-sector-time-min", result[1]);
			}
			# only for mp
	...
	var fl = getprop("/Honda-NSF/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/Honda-NSF/last-lap-time") or 0;
	...
	if(fl > 0){
		...
		setprop("Honda-NSF/fastest-lap-time-sec", result[0]);
		setprop("Honda-NSF/fastest-lap-time-min", result[1]);
		setprop("Honda-NSF/fastest-lap-time-hour", result[2]);
	}
	if(lapt > 0){
		var result = calc_time(lapt);
		setprop("Honda-NSF/this-lap-time-sec", result[0]);
		setprop("Honda-NSF/this-lap-time-min", result[1]);
		setprop("Honda-NSF/this-lap-time-hour", result[2]);
	}
	# only for mp
...
var find_marker = func{
	...
	if(getprop("/Honda-NSF/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/Honda-NSF/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/Honda-NSF/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/Honda-NSF/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/Honda-NSF/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/Honda-NSF/"~pa~"/sector["~ln~"]/start-time") or 0;
		var lastsectorendtime = getprop("/Honda-NSF/"~pa~"/sector["~thissector.getValue()~"]/start-time") or 0;
		...
		setprop("/Honda-NSF/"~pa~"/sector["~ln~"]/last-time", lasttime);
		if(lasttime > 0 and lasttime < fastesttime or fastesttime == 0) setprop("/Honda-NSF/"~pa~"/sector["~ln~"]/fastest-time", lasttime);
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/Honda-NSF/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
		  setprop("/Honda-NSF/last-lap-time", totallapresult);
		  var fastestlap = getprop("/Honda-NSF/"~pa~"/fastest-lap") or 0;
		  if(totallapresult > 0 and totallapresult < fastestlap or fastestlap == 0) setprop("/Honda-NSF/"~pa~"/fastest-lap", totallapresult);
		  setprop("/Honda-NSF/"~pa~"/lap["~racelap.getValue()~"]/actual-time", totallapresult);
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/Honda-NSF/"~pa);
		
		setprop("/Honda-NSF/last-lap-time",0);
